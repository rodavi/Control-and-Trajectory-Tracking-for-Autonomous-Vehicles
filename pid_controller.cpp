...
double PID::TotalError() {
   ...
   */
    double control;
  	// Check to make sure control input does not become inf
  	if (this->new_delta_time > 0){
      control = -this->Kp * this->cte - this->Kd * this->diff_cte / this->new_delta_time - this->Ki * this->int_cte * this->new_delta_time;
    }
  	else{
      control = -this->Kp * this->cte;
    }
      
  	// Check to limit the control to limits
